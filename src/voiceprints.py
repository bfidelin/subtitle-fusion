...
class VoiceprintStore:
    """Small JSON voiceprint store using conservative centroid matching.

    The store is intentionally model-agnostic: embeddings are supplied by the
    diarization backend. A match must clear both an absolute threshold and the
    margin over the second-best identity. Unknown is preferable to a false name.
    """
    ...
    def enroll(self, name: str, embedding: list[float], *, max_samples: int = 12) -> None:
        if not embedding:
            return
